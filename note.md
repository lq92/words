...
## Gulp
  ### 通配符——一种匹配模式，允许匹配到多个文件
    *.sass: *匹配当前目录任意文件，所以*.sass匹配当前目录下所有的sass文件
    **/*.sass: 匹配当前目录及其子目录下的所有sass文件
    !test.sass: !号移除匹配的文件，这里将移除test.sass
    *.+(sass|scss): 匹配多个选项，这里匹配sass和scss文件
